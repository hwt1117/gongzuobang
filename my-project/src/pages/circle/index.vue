<template>
  <div class="all">
    ...
    <div class="header">
      ...
      <div>
        <scroll-view class="header-shequ" scroll-x="true">
          <div class="header-shequ-box" v-for="(item, index) in merchant" :key="index">
            <div class="header-shequn-box-i">
              ...
              <div style="font-size:12px;text-align:center">{{item.shequn}}</div>
            </div>
          </div>
    ...
    <div :hidden="activeIndex != 0">
      <div class="news" v-for="item in merchant" :key="item.id" @click="getDetail()">
...
export default {
  ...
  methods: {
    ...
    getDetail() {
      wx.navigateTo({
        url: "/pages/circle_newsdetails/main"
      });
    }
  },
  onLoad() {
